...
<br>
<button class="btn"><i class="fa fa-folder"></i> Εργασίες</button></a>
<br>
...
<div class="text1" >
<a href="addHomework.php"><button class="btn" align="right">Προσθήκη νέας εργασίας</button></a> 
</div>
...
if(isset($_GET['delete_id'])) {
    ...
    $sql = "DELETE FROM homeworks WHERE id = $delete_id";

    if ($data->query($sql) === TRUE) {
        echo "Η εργασία διαγράφηκε με επιτυχία";
    } else {
        echo "Σφάλμα κατά τη διαγραφή της εργασίας: " . $data->error;
    }
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<div align='left'>";
	   echo "<h2>Εργασία" . $row["id"] . " <a href='?delete_id=" . $row["id"] . "'>[Διαγραφή]</a> <a href='updateHomework.php?id=" . $row["id"] . "'>[Επεξεργασία]</a> </h2>";
        echo "</div>";
        echo "</div>";
...
if (isset($row["goals"])) {
    ...
    echo "</ul>";
       ...
       echo "<p><b>Εκφώνηση:</b></p>";
		echo "<p><a href='" . $row["details"] . "'>Κατεβάστε την εκφώνηση της εργασίας από εδώ</a></p>";  
...
if (isset($row["deadline"])) {
    echo '<p><span class="clrRed"><b>Ημερομηνία Παράδοσης:</b> ' . $row["deadline"] . '</span></p>';
}
